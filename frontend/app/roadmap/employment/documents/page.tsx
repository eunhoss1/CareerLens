  ...
  return (
    <PageShell>
      ...
      <PageHeader
        ...
        title="AI 문서 분석"
        description="플래너 과제의 기대 산출물과 검증 기준을 기준으로 텍스트, GitHub repository, PDF/DOCX 제출물을 점검하고 검증 배지를 발급합니다."
      ...
      <section className="lens-container py-6">
        {isLoadingTasks ? (
          ...
          <EmptyState
            ...
            description="맞춤채용정보 진단 또는 전체 공고에서 로드맵을 먼저 생성하면 과제 기준 문서 분석을 사용할 수 있습니다."
            ...
            <aside className="space-y-4">
              <Card className="p-5">
                <div className="flex flex-wrap gap-2">
                  <Badge tone="brand">PlannerTask 연결</Badge>
                  <Badge tone="muted">GitHub repository 전용</Badge>
                  <Badge tone="muted">PDF/DOCX 지원</Badge>
                </div>
                <h2 className="mt-4 text-xl font-semibold text-night">분석 기준 선택</h2>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  제출물을 과제의 기대 산출물과 검증 기준에 맞춰 분석합니다. GitHub는 프로필 URL이 아니라 실제 프로젝트 repository URL만 허용합니다.
                </p>
              ...
              {selectedTask && (
                <Card className="p-5">
                  <p className="text-xs font-bold tracking-[0.16em] text-brand">TASK STANDARD</p>
                  <h3 className="mt-3 text-lg font-semibold text-night">{selectedTask.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-slate-600">{selectedTask.description}</p>
                  <div className="mt-4 space-y-3 text-sm">
                    <InfoBlock title="기대 산출물" value={selectedTask.expected_outputs} />
                    <InfoBlock title="검증 기준" value={selectedTask.verification_criteria} />
                    <div className="flex flex-wrap gap-2">
            ...
            <div className="space-y-4">
              <Card className="p-5">
                <div className="grid gap-2 sm:grid-cols-3">
                  <ModeButton active={submitMode === "TEXT"} onClick={() => setSubmitMode("TEXT")} label="텍스트 입력" />
                  <ModeButton active={submitMode === "GITHUB"} onClick={() => setSubmitMode("GITHUB")} label="GitHub URL" />
                  <ModeButton active={submitMode === "FILE"} onClick={() => setSubmitMode("FILE")} label="PDF/DOCX 업로드" />
                </div>

                <div className="mt-5">
                  {submitMode === "TEXT" && (
                    <label className="block">
                      <span className="text-sm font-semibold text-night">분석할 문서 내용</span>
                      <textarea
                        className="mt-2 min-h-[360px] w-full border border-line bg-white p-4 text-sm leading-6 text-ink outline-none focus:border-brand"
                        placeholder="이력서 요약, 자기소개서 초안, README 내용, 포트폴리오 설명 등을 붙여넣으세요."
                        value={submittedText}
                  ...
                  {submitMode === "GITHUB" && (
                    <div className="space-y-4">
                      <label className="block">
                        ...
                        <input
                          className="mt-2 h-11 w-full border border-line bg-white px-3 text-sm outline-none focus:border-brand"
                      ...
                      <label className="block">
                        ...
                        <textarea
                          className="mt-2 min-h-[160px] w-full border border-line bg-white p-4 text-sm leading-6 outline-none focus:border-brand"
                  ...
                  {submitMode === "FILE" && (
                    <div className="border border-line bg-panel p-5">
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-600">
                        이력서, 자기소개서, 프로젝트 사례서, README를 PDF/DOCX로 올리면 텍스트를 추출해 과제 기준으로 분석합니다. 최대 5MB까지 지원합니다.
                      </p>
                      <input
                        className="mt-4 block w-full text-sm text-slate-700 file:mr-4 file:border file:border-line file:bg-white file:px-4 file:py-2 file:text-sm file:font-semibold file:text-night"
                        type="file"
  ...
  return (
    <button
      ...
      className={`border px-4 py-3 text-sm font-semibold transition ${
        active ? "border-night bg-night text-white" : "border-line bg-white text-slate-700 hover:border-night"
      }`}
      onClick={onClick}
    >
      {label}
