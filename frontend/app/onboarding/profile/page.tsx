      ...
      <section className="lens-container grid gap-5 py-6 lg:grid-cols-[340px_1fr]">
        ...
        <div className="space-y-5">
          ...
          <Section step="04" title="지원 조건과 증빙">
            <TextInput label="희망 연봉" value={profile.expected_salary_range ?? ""} onChange={(event) => setProfile({ ...profile, expected_salary_range: event.target.value })} placeholder="예: USD 100k-130k / Not specified" />
  ...
  return (
    <Card className="rounded-2xl p-5">
      <div className="mb-5 flex gap-4">
        <span className="mt-1 h-fit rounded-lg border border-line bg-panel px-2.5 py-1 text-xs font-bold text-brand">{step}</span>
        <div>
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <button
      ...
      className={`mt-6 flex h-10 items-center justify-between rounded-xl border px-3 text-sm font-semibold ${
...
function PriorityToggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    ...
    >
      ...
      <span className="mt-1 block text-xs">{checked ? "선택됨" : "미선택"}</span>
    </button>
  );
}
    ...
    <div className="md:col-span-2">
      ...
      <div className="mt-1 rounded-xl border border-line bg-white p-2">
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <button key={tag} type="button" onClick={() => onChange(tags.filter((item) => item !== tag))} className="rounded-lg bg-[#e8f2f1] px-3 py-1 text-sm font-semibold text-brand">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {suggestions.map((suggestion) => (
          <button key={suggestion} type="button" onClick={() => addTag(suggestion)} className="rounded-lg border border-line px-3 py-1 text-xs font-semibold text-slate-600 hover:border-brand hover:text-brand">
